...
**Status:** Active source of truth · Neutral minimal surfaces in production  
**Version:** 1.2.0  
...
The visual language combines ancient Hebrew symbolism with a calm contemporary product system: warm white day surfaces, warm charcoal night surfaces, restrained sacred purple accents, and contemplative navy text. The app should feel reverent, simple, welcoming, and premium without becoming heavy or ornamental.

---

## 2. Design Principles

1. **Sacred Simplicity** — Strip away decorative noise. Every visual decision must serve clarity.
2. **Warm Neutral Foundation** — Primary screens live on warm white or warm dark-gray backgrounds, never cold white and never pure black.
3. **Brand As Accent, Not Container** — Purple and navy are primarily for CTAs, active states, icons, outlines, selected chips, and focused emphasis, not for large section boxes.
4. **Flat Surfaces Over Effects** — No gradients, radial overlays, glassmorphism, or frosted surfaces on primary UI blocks.
5. **Compact Mobile Rhythm** — Prefer tighter, calmer section spacing and smaller card footprints over oversized hero treatments.
6. **Token-First Design** — New visual patterns should be expressed through shared `--theme-*` or `--brand-*` variables before being repeated in components.
7. **Telegram-Native Practicality** — Respect safe areas, keep the shell predictable, and favor lightweight surfaces that render cleanly in Telegram WebView.

---

## 3. Active Token System

The active design token source of truth is `apps/miniapp/src/styles/index.css`.

### Brand Tokens

| Token                | Value     | Usage                                             |
| -------------------- | --------- | ------------------------------------------------- |
| `--brand-purple`     | `#7D5AF2` | Primary CTA, active nav bubble, selected emphasis |
| `--brand-navy`       | `#09194D` | Strong text, contrast, map-related emphasis       |
| `--brand-accent`     | `#5C4DD9` | Secondary purple emphasis, outlined accent states |
| `--brand-text`       | `#0F172A` | Primary light-mode content text                   |
| `--brand-text-muted` | `#475569` | Secondary light-mode text                         |
| `--brand-border`     | `#CBD5E1` | Generic subtle border reference                   |
| `--brand-success`    | `#10B981` | Positive states                                   |
...
| ------------------------ | ------------------------ | --------------------- | ---------------------- |
| `--theme-bg-main`        | `#F7F2EB`                | `#26211D`             | Main page background   |
| `--theme-bg-solid`       | `#F7F2EB`                | `#26211D`             | `body` background      |
| `--theme-nav-bg`         | `rgba(247,242,235,0.96)` | `rgba(38,33,29,0.94)` | Bottom nav bar         |
| `--theme-nav-shadow`     | soft warm shadow         | darker warm shadow    | Bottom nav separation  |
| `--theme-text-primary`   | navy                     | warm light text       | Primary content text   |
| `--theme-text-secondary` | slate muted              | warm muted            | Secondary content text |

### Surface Tokens

| Token                             | Light             | Dark                | Usage                         |
| --------------------------------- | ----------------- | ------------------- | ----------------------------- |
| `--theme-card-bg`                 | `#FFFAF4`         | `#332D29`           | Main elevated card surface    |
| `--theme-card-border`             | subtle navy alpha | subtle warm alpha   | Card outline                  |
| `--theme-card-shadow`             | soft warm shadow  | soft dark shadow    | Card elevation                |
| `--theme-surface-warm`            | `#FFFAF4`         | `#332D29`           | Base warm surface             |
| `--theme-surface-warm-muted`      | `#F3ECE3`         | `#2B2622`           | Muted neutral section surface |
| `--theme-surface-warm-border`     | subtle navy alpha | subtle warm alpha   | Neutral inner border          |
| `--theme-surface-warm-text`       | `--brand-text`    | `--brand-text-dark` | Text on warm surfaces         |
| `--theme-surface-warm-muted-text` | accent purple     | `#D4C7FF`           | Highlighted muted text        |

### Action Tokens

| Token                             | Usage                            |
| --------------------------------- | -------------------------------- |
| `--theme-button-primary-bg`       | Primary filled action background |
| `--theme-button-primary-border`   | Primary filled action border     |
| `--theme-button-primary-text`     | Primary filled action text       |
| `--theme-button-primary-shadow`   | CTA elevation                    |
| `--theme-button-secondary-bg`     | Secondary filled surface         |
| `--theme-button-secondary-border` | Secondary action outline         |
| `--theme-button-secondary-text`   | Secondary action text            |
...
### Overlay And Sheet Tokens

| Token                                                                                           | Usage                     |
| ----------------------------------------------------------------------------------------------- | ------------------------- |
| `--theme-toggle-bg` / `--theme-toggle-border` / `--theme-toggle-icon` / `--theme-toggle-shadow` | Theme toggle shell button |
| `--theme-sheet-shadow`                                                                          | Bottom sheet elevation    |
